...
static WalkResult emitArithOp(mlir::raw_indented_ostream &os, Operation *op) {
  ...
  return TypeSwitch<Operation *, WalkResult>(op)
      ...
      // NOTE: The following two ops are generated by ControlFlowToSCF
      .Case<arith::ConstantOp>([&](auto constantOp) {
        valToPy(os, constantOp.getResult());
        os << " = ";
        auto value = constantOp.getValue();
        if (!value.getType().isInteger()) {
          op->emitError()
              << "CoreDSL::emitArithOp does not support non integer constants!";
          return WalkResult::interrupt();
        }
        auto intValue = cast<IntegerAttr>(value);
        os << intToPy(intValue.getAPSInt()) << '\n';
        return WalkResult::advance();
      })
      .Case<arith::IndexCastUIOp>([&](arith::IndexCastUIOp castOp) {
        valToPy(os, castOp.getResult());
        os << " = ";
        valToPy(os, castOp.getOperand());
        // TODO: index type is not necessarily 64 bit. Also, is signed or
        // unsigned better
        os << ".cast(64, True)\n";
        return WalkResult::advance();
      })
...
static WalkResult emitSCFOp(mlir::raw_indented_ostream &os, Operation *op) {
  ...
  return TypeSwitch<Operation *, WalkResult>(op)
      ...
      // NOTE: Only generated by ControlFlowToSCF
      .Case<scf::IndexSwitchOp>([&](scf::IndexSwitchOp switchOp) {
...
struct CoreDSLToPy
  ...
  void runOnOperation() override {
    ...
    inlinePM.addPass(createCoreDSLLiftCFToSCF());
